...
def local_storage_get():
    ...
    try:
        ...
        raw = streamlit_js_eval(
            js_expressions=
                "localStorage.getItem('noor_cyber_pending_v2')",
            key="noor_local_get_v2"
        )
...
def local_storage_set(records):
    ...
    try:
        ...
        return streamlit_js_eval(
            ...
            key=
                f"noor_local_set_{uuid.uuid4().hex}"
        )
...
def make_pdf(df):
    ...
    for pi, page in enumerate(
        ...
        objects.append(
            ...
            f"<< /Type /Page "
            f"/Parent 2 0 R "
            f"/MediaBox [0 0 842 595] "
            f"/Contents {content_ids[pi]} 0 R "
            f"/Resources << /Font << "
            f"/F1 {font_id} 0 R "
            f">> >>"
    ...
    if st.button(
        ...
        if local_df.empty:
            ...
            if not failed:
                ...
                st.error(
                    "Some entries could not be backed up. "
...
with a2:
    ...
    if st.button(
        ...
        if err:
            ...
            pending_local = (
                st.session_state.records_cache[
                    st.session_state.records_cache["_source"]
                    == "local"
                ].copy()
            ...
            st.success(
                "✅ Records loaded from Google Sheet. "
                "Pending local entries were kept safe."
            )
...
if st.session_state.get(
    ...
    st.warning(
        ...
        "New entries will remain safely in local storage "
        "until backup."
...
prev_col, date_col, next_col = st.columns(
    [1, 5, 1]
)
    ...
    if st.button(
        ...
        if (
            ...
            new_row = {
                ...
                "_row_number":
                    -st.session_state.local_counter,
            ...
            st.session_state.success_message = (
                ...
                "Entry is safely stored locally. "
                "Press BACKUP TO SHEET to sync it."
